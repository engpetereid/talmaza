    ...
    <div class="px-4 py-8 mx-auto space-y-8 max-w-7xl sm:px-6 lg:px-8">
        ...
        @if($type == 'weekly')
            ...
            <div class="grid grid-cols-1 gap-6 lg:grid-cols-3">
                            ...
                            <div class="flex flex-col gap-3 group {{ $isReadOnly ? 'p-4 border-2 border-indigo-100 bg-indigo-50/30 rounded-2xl' : '' }}">
                                ...
                                @if($isReadOnly)
                                    ...
                                    <div class="flex items-start gap-3 pt-2 mt-1 border-t border-indigo-200/50">
                                        <div class="px-2 py-1 mt-2 text-xs font-black text-indigo-800 bg-indigo-200 rounded shrink-0">رد جديد:</div>
                ...
                <!-- Visitation Hours -->
                <div class="flex flex-col p-6 bg-white border border-gray-200 shadow-sm lg:col-span-1 rounded-3xl">
                    <div class="flex flex-col items-center justify-center {{ $isReadOnly ? 'mb-6' : 'h-full' }}">
                    ...
                    <!-- Threaded Replies for Visitation Hours -->
                    @if($isReadOnly)
                        <div class="flex-grow w-full mt-2 space-y-3">
                            @foreach($visitation_replies ?? [] as $reply)
                                <div class="flex items-start gap-3 p-4 rounded-xl {{ $reply['role'] == 'admin' ? 'bg-indigo-100 border border-indigo-200' : 'bg-blue-100 border border-blue-200' }}">
                            ...
                            <!-- Input for NEW reply -->
                            <div class="flex items-start gap-3 pt-3 mt-4 border-t border-blue-200/50">
                                <div class="px-2 py-1 mt-2 text-xs font-black text-blue-800 bg-blue-200 rounded shrink-0">رد جديد:</div>
                                <textarea wire:model="visitation_new_reply" rows="2"
                                          ...
                                          placeholder="تعليق على الافتقاد..."></textarea>
                            </div>
                        </div>
                    @endif
                </div>
                        ...
                        <div class="group bg-gray-50 p-5 rounded-2xl border-2 {{ $isReadOnly ? 'border-green-200 bg-green-50/30' : 'border-gray-200' }}">
                                    ...
                                    <div class="w-full">
                                        ...
                                        <div class="flex justify-end mt-2">
                                            <button wire:click="removeItem('weekly_achievements', {{ $index }})"
                                                ...
                                                <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" st
                                                      roke-width="2.5"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>
                                                حذف
                                            </button>
                                        </div>
                                    </div>
                            ...
                            @if($isReadOnly)
                                ...
                                <div class="flex items-start gap-3 pt-3 mt-4 border-t border-green-200/50">
                                    <div class="px-2 py-1 mt-2 text-xs font-black text-green-800 bg-green-200 rounded shrink-0">رد جديد:</div>
        ...
        <div class="bg-gradient-to-br from-indigo-800 to-purple-900 text-white p-6 md:p-8 rounded-[2rem] shadow-xl relative overflow-hidden mb-8 animate-fade-in-down">
            <div class="relative z-10">
                ...
                <div class="grid grid-cols-2 gap-4 mb-8 md:grid-cols-4">
                    ...
                    <div class="p-3 text-center border bg-white/10 rounded-2xl backdrop-blur-md border-white/10"><span class="block mb-1 text-3xl font-bold">{{ $stats_snapshot['mass'] ?? 0 }}%</span><span class="text-[10px] opacity-60 font-bold">القداس</span></div>
                    <div class="p-3 text-center border bg-white/10 rounded-2xl backdrop-blur-md border-white/10"><span class="block mb-1 text-3xl font-bold">{{ $stats_snapshot['servants'] ?? 0 }}%</span><span class="text-[10px] opacity-60 font-bold">اجتماع الخدام</span></div>
                    <div class="p-3 text-center border bg-white/10 rounded-2xl backdrop-blur-md border-white/10"><span class="block mb-1 text-3xl font-bold">{{ $stats_snapshot['vespers'] ?? 0 }}%</span><span class="text-[10px] opacity-60 font-bold">عشية</span></div>
                    <div class="p-3 text-center border bg-white/10 rounded-2xl backdrop-blur-md border-white/10"><span class="block mb-1 text-3xl font-bold">{{ $stats_snapshot['tasbeha'] ?? 0 }}%</span><span class="text-[10px] opacity-60 font-bold">تسبحة</span></div>
                    <div class="p-3 text-center border bg-white/10 rounded-2xl backdrop-blur-md border-white/10"><span class="block mb-1 text-3xl font-bold">{{ $stats_snapshot['reading'] ?? 0 }}%</span><span class="text-[10px] opacity-60 font-bold">قراءة</span></div>
                    <div class="p-3 text-center border bg-white/10 rounded-2xl backdrop-blur-md border-white/10"><span class="block mb-1 text-3xl font-bold">{{ $stats_snapshot['weekly_kholwa'] ?? 0 }}%</span><span class="text-[10px] opacity-60 font-bold">الخلوة الاسبوعية</span></div>
                    <div class="p-3 text-center border bg-white/10 rounded-2xl backdrop-blur-md border-white/10"><span class="block mb-1 text-3xl font-bold">{{ $stats_snapshot['sermon'] ?? 0 }}%</span><span class="text-[10px] opacity-60 font-bold">العظة</span></div>
                    <div class="p-3 text-center border bg-white/10 rounded-2xl backdrop-blur-md border-white/10"><span class="block mb-1 text-3xl font-bold">{{ $stats_snapshot['altar'] ?? 0 }}%</span><span class="text-[10px] opacity-60 font-bold">مذبح عائلى</span></div>
                </div>
                    ...
                    <div class="overflow-x-auto">
                        <table class="w-full text-sm text-center">
                            ...
                            <tr>
                                ...
                                <th class="p-4">تسبحة</th>
                                <th class="p-4">مشاركة الخلوة</th>
                                <th class="p-4">قراءة</th>
                                <th class="p-4">اجتماع الخدام</th>
                            ...
                            @foreach($members_monthly_stats as $stat)
                                <tr class="hover:bg-white/5 transition-colors {{ !$stat['is_active'] ? 'opacity-50 bg-black/10' : '' }}">
                                    ...
                                    <td class="p-3">{{ $stat['has_vespers'] }}%</td>
                                    <td class="p-3">{{ $stat['has_tasbeha'] }}%</td>
                                    <td class="p-3">{{ $stat['kholwa_count'] }}%</td>
                                    <td class="p-3">{{ $stat['has_reading'] }}%</td>
                                    <td class="p-3">{{ $stat['has_servants_meeting'] }}%</td>
                                    <td class="p-3">{{ $stat['talmaza_training_count'] }}%</td>
                    ...
                    <div class="p-4 transition-colors border-2 border-gray-100 bg-gray-50 rounded-2xl hover:border-red-200">
                        ...
                        @if($isReadOnly && !empty($members_notes[$member->id]['text']))
                            ...
                            <div class="flex items-start gap-2 mt-3">
                                <span class="text-[10px] bg-red-100 text-red-800 font-black px-2 py-1 rounded mt-1 shrink-0">رد:</span>
